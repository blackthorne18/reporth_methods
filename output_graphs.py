...
def process():
    ...
    if os.path.isdir(outdir) and CACHE:
        ...
        isolates = pickle.load(open(outdir + "isolates", "rb"))
        cliques = pickle.load(open(outdir + "cliques", "rb"))
        connects = pickle.load(open(outdir + "connects", "rb"))
        return 0
...
def summary_plots(summary_types):
